...
In my `.gitignore` file, I've included a `todo.yml` with semi-structured notes about topics I may wish to write about in the future but notes which I don't want globally visible. I have also added the rule `docs/_posts/*-draft.markdown` so that `.markdown` content in draft status is automatically ignored by `git`.